...
L1 是真理底座（[PRIN-ARCH-3] 不可变原则 + Markdown 是 ground truth）。即使最激进的全量档位,可重建的只有衍生数据(L2/L3、SQLite 索引、关系、词频表),L1 的 Markdown 内容与 patches 修订历史绝不重建。
