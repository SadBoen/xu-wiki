...
> **覆盖范围**：9 份文档（00 索引 + 01-09 模块）对应系统的核心模块。
...
| **L2 知识库本体** | wiki 里的所有节点 | ingest / query / revise / delete-node / table / gallery / report |
...
- 格式：8 位大写字母数字 `{counter}{random}`，counter 为同秒内单调递增（上限 1296/秒），random 为 6 位随机字符
- 生成时即保证唯一（同秒内确定性），溢出时切纯随机（碰撞概率可忽略）
...
本套文档（00-09）由一份**早期手写设计稿**提炼而来——原稿含三层架构、检索 SOP、强 Schema、50 条关系、rebuild 粒度等核心想法，以及一组具体参数值。
...
> 注：「50 条关系上限」「L1 不可变性」也是常见踩坑点，但已在跨文档核心原则 #3 / #7 覆盖，此处不重复列出。
