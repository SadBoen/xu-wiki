//! Command implementations 鈥?Rust owns all business logic.
...
// ======== DOCTOR (filesystem + database integrity) ========
...
pub fn cmd_doctor(wiki_path: &str) -> Value {
    ...
    // --- Layer 1: filesystem ---
    checks.push(json!({"check":"raws_dir","ok":root.join("raws").is_dir(),"group":"fs"}));
    checks.push(json!({"check":"xu_dir","ok":xu.is_dir(),"group":"fs"}));
    checks.push(json!({"check":"config_yaml","ok":xu.join("config.yaml").exists(),"group":"fs"}));
    checks.push(json!({"check":"wiki_db","ok":xu.join("wiki.db").metadata().map(|m|m.len()).unwrap_or(0)>0,"group":"fs"}));
    checks.push(json!({"check":"state_json","ok":xu.join("state.json").exists(),"group":"fs"}));
    ...
    if let Ok(ref db) = db {
        ...
        checks.push(json!({"check":"page_total","ok":true,"detail":total_cnt,"group":"stats"}));
        checks.push(json!({"check":"page_active","ok":true,"detail":active_cnt,"group":"stats"}));
        checks.push(json!({"check":"page_deactivated","ok":true,"detail":dead_cnt,"group":"stats"}));
        ...
        checks.push(json!({"check":"entity_count","ok":true,"detail":entity_cnt,"group":"stats"}));
        checks.push(json!({"check":"list_count","ok":true,"detail":list_cnt,"group":"stats"}));
        checks.push(json!({"check":"report_count","ok":true,"detail":report_cnt,"group":"stats"}));
        ...
        checks.push(json!({"check":"relation_count","ok":true,"detail":rel_cnt,"group":"stats"}));
        ...
        checks.push(json!({"check":"broken_from_uid","ok":broken_from_cnt==0,"detail":format!("{} orphan(s)", broken_from_cnt),"group":"integrity"}));
        checks.push(json!({"check":"broken_to_uid","ok":broken_to_cnt==0,"detail":format!("{} dangling(s)", broken_to_cnt),"group":"integrity"}));
        ...
        checks.push(json!({"check":"orphan_entities","ok":orphan_cnt==0,"detail":format!("{} without relations", orphan_cnt),"group":"integrity"}));
        ...
        checks.push(json!({"check":"empty_lists","ok":empty_list_cnt==0,"detail":format!("{} empty list(s)", empty_list_cnt),"group":"integrity"}));
    } else {
        checks.push(json!({"check":"db_open","ok":false,"detail":"cannot open database","group":"fs"}));
    }
    ...
    if reg_path.exists() {
        ...
        checks.push(json!({"check":"registry_entry","ok":reg_ok,"detail":if reg_ok {"found"} else {"missing"},"group":"registry"}));
    } else {
        checks.push(json!({"check":"registry_file","ok":false,"detail":"~/.xu-wiki/config.yaml not found","group":"registry"}));
    }

    let issues = checks.iter().filter(|c| !c["ok"].as_bool().unwrap_or(false)).count();
    let stats: Value = checks.iter().filter(|c| c["group"]=="stats").map(|c| {(c["check"].as_str().unwrap_or("").to_string(), c["detail"].as_i64().map(|n| json!(n)).unwrap_or(json!(c["detail"].as_str().unwrap_or(""))))}).collect::<serde_json::Map<_,_>>().into();

    if issues == 0 {
        response::success(json!({"checks":checks,"issues":0,"wiki":wiki_path,"stats":stats}), "all checks passed")
    } else {
        response::warning(json!({"checks":checks,"issues":issues,"wiki":wiki_path,"stats":stats}), &format!("{} issue(s)", issues))
    }
}
...
fn build_reflection(db: &crate::db::Db, core_keywords: &[String], top: &[Value]) -> Value {
    ...
    json!({
        ...
        "hint": if has_pages && existing_entities.is_empty() {
            format!("{} page(s) found 鈥?consider extracting entities with: xu entity-create --wiki <w> --title <name> --source-page <uid>", top.len())
        } else if top.len() >= 2 && existing_lists.is_empty() {
            "multiple results share a theme 鈥?consider: xu list-create".into()
        } else {
            "".into()
        }
    })
