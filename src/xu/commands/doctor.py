...
def _summarize(checks_report: dict) -> dict:
    ...
    for r in checks_report.values():
        for issue in r.get("issues", []):
            total += 1
            by_layer[issue.get("layer", "cross")] = (
                by_layer.get(issue.get("layer", "cross"), 0) + 1
            )
...
def cmd_doctor(args) -> dict:
    ...
    if kind in ("doctor", "doctor-all"):
        ...
        if fix:
            ...
            data["post_fix"] = {
                "residual_issues": post["total_issues"],
    ...
    hints = (
        []
        if (summary["total_issues"] == 0 or fix)
        else [
            f"re-run with --fix to repair {summary['auto_fixable']} auto-fixable issue(s)"
        ]
    )
...
def _check_files(ctx, fix) -> dict:
    ...
    for md in ctx.page_dir.rglob("*.md"):
        ...
        if rel not in fm_paths:
            issues.append(
                {
                    "problem": "orphan md file (not in frontmatter)",
                    "path": rel,
                    "layer": "Page",
                    "fixable": False,
                }
    ...
    for md_path, fm_dict, _ in _all_frontmatter_nodes(ctx):
        ...
        if len(rels) > MAX_EDGES:
            issues.append(
                {
                    "from_uid": src,
                    "problem": f"edge count {len(rels)} > {MAX_EDGES}",
                    "layer": "cross",
                    "fixable": True,
                }
            )
        ...
        if positions != list(range(len(positions))):
            issues.append(
                {
                    "from_uid": src,
                    "problem": "non-contiguous positions",
                    "positions": positions,
                    "layer": "cross",
                    "fixable": True,
                }
        ...
        for r in rels:
            if r.get("to_uid") not in all_uids:
                issues.append(
                    {
                        "from_uid": src,
                        "problem": "dangling relation target",
                        "to_uid": r.get("to_uid"),
                        "layer": "cross",
                        "fixable": True,
                    }
    ...
    for md_path, fm_dict, body in _all_frontmatter_nodes(ctx):
        ...
        if actual != stored_hash:
            issues.append(
                {
                    "uid": fm_dict.get("uid"),
                    "problem": "Page content_hash mismatch (tampered)",
                    "expected": stored_hash[:12],
                    "actual": actual[:12],
                    "layer": "Page",
                    "fixable": False,
                }
    ...
    for md_path, fd, _ in _all_frontmatter_nodes(ctx):
        ...
        if not evidence_list:
            issues.append(
                {
                    "report_uid": uid,
                    "problem": "report with zero evidence (BAN-ARCH-5)",
                    "layer": "Report",
                    "fixable": False,
                }
            )
        ...
        for ref in evidence_list:
            ...
            if active is None:
                ...
                issues.append(
                    {
                        "report_uid": uid,
                        "problem": "evidence ref points to inactive node",
                        "ref_uid": ref_uid,
                        "layer": "Report",
                        "fixable": True,
                    }
...
def _check_node_path_organization(ctx, fix) -> dict:
    ...
    for p in root_page_dir.glob("*.md"):
        try:
            ...
            if (
                uid
                and fm_dict.get("layer") == "Page"
                and fm_dict.get("active", True)
                and not (fm_dict.get("node_path") or "").strip()
            ):
                root_uids.add(uid)
        except Exception:
            continue
    ...
    return success(
        {
            ...
            "cleaned_list_refs": list_refs,
            "cleaned_evidence_refs": evidence_refs,
            "cleaned_relation_refs": rel_refs,
            "forced": args.force,
        },
...
def cmd_rebuild(args) -> dict:
    ...
    return success(
        {"granularity": gran, "actions": actions},
