//! doc_parser pyo3 bridge — call Python markitdown from Rust.
...
/// Extensions supported by the doc parser.
const SUPPORTED: &[&str] = &[".pdf", ".docx", ".pptx", ".html", ".htm"];
...
/// Parse a document via Python markitdown, returning markdown text.
pub fn parse_document(path: &Path) -> Result<ParseResult, ParserError> {
    ...
    Python::with_gil(|py| {
        ...
        let module = py.import_bound("doc_parser")?;
        let func = module.getattr("parse_document")?;
        let result = func.call1((&path_str,))?;
        let content: String = result.extract()?;

        Ok(ParseResult::new(content, "markitdown"))
    })
}
